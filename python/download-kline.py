...
def download_monthly_klines(trading_type, symbols, num_symbols, intervals, years, months,
    ...
    _logger.info("Starting monthly klines — %d symbols", num_symbols)

    for idx, symbol in enumerate(symbols, 1):
        _logger.info("[%d/%d] Monthly klines: %s", idx, num_symbols, symbol)
...
def download_daily_klines(trading_type, symbols, num_symbols, intervals, dates,
                           start_date, end_date, folder, checksum, symbol_timeout):
    date_range = None
    if start_date and end_date:
        date_range = start_date + " " + end_date
    ...
    # Only intervals supported for daily granularity
    intervals = list(set(intervals) & set(DAILY_INTERVALS))
    _logger.info("Starting daily klines — %d symbols", num_symbols)

    for idx, symbol in enumerate(symbols, 1):
        _logger.info("[%d/%d] Daily klines: %s", idx, num_symbols, symbol)
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(
    ...
    if args.dates:
        ...
        if args.skip_monthly == 0:
            download_monthly_klines(
                args.type, symbols, num_symbols, args.intervals,
                args.years, args.months, args.startDate, args.endDate,
                args.folder, args.checksum, args.symbol_timeout
            )
