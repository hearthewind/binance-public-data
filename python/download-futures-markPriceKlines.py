...
"""
  ...
  e.g. STORE_DIRECTORY=/data/ ./download-futures-markPriceKlines.py
...
def download_monthly_markPriceKlines(trading_type, symbols, num_symbols, intervals, years, months,
    ...
    _logger.info("Starting monthly markPriceKlines — %d symbols", num_symbols)

    for idx, symbol in enumerate(symbols, 1):
        _logger.info("[%d/%d] Monthly markPriceKlines: %s", idx, num_symbols, symbol)
...
def download_daily_markPriceKlines(trading_type, symbols, num_symbols, intervals, dates,
                                    start_date, end_date, folder, checksum, symbol_timeout):
    date_range = None
    if start_date and end_date:
        date_range = start_date + " " + end_date
    ...
    _logger.info("Starting daily markPriceKlines — %d symbols", num_symbols)

    for idx, symbol in enumerate(symbols, 1):
        _logger.info("[%d/%d] Daily markPriceKlines: %s", idx, num_symbols, symbol)
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(
