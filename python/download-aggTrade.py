...
def download_monthly_aggTrades(trading_type, symbols, num_symbols, years, months,
    ...
    _logger.info("Starting monthly aggTrades — %d symbols", num_symbols)

    for idx, symbol in enumerate(symbols, 1):
        _logger.info("[%d/%d] Monthly aggTrades: %s", idx, num_symbols, symbol)
...
def download_daily_aggTrades(trading_type, symbols, num_symbols, dates,
                              start_date, end_date, folder, checksum, symbol_timeout):
    date_range = None
    if start_date and end_date:
        date_range = start_date + " " + end_date
    ...
    _logger.info("Starting daily aggTrades — %d symbols", num_symbols)

    for idx, symbol in enumerate(symbols, 1):
        _logger.info("[%d/%d] Daily aggTrades: %s", idx, num_symbols, symbol)
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(
