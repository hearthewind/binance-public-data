...
INDEX_NAME = 'trades_symbol_time_idx'
TMP_RAW_TABLE = 'tmp_trade_raw'


def ensure_tables(conn, chunk_days: int = 30, compress_after_days: int = 90):
    cur = conn.cursor()
...
def copy_csv_and_flush(csv_stream, token: str, conn):
...
def process_monthly(
    ...
    # Trades have no freq subdirectory: data/spot/monthly/trades/TOKEN/DATE_RANGE/
    ...
    errors = run_worker_pool(
        ...
        process_file_fn=lambda conn, stream, token: copy_csv_and_flush(stream, token, conn),
    )
